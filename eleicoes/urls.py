...
urlpatterns = patterns(
    ...
    url(r'^santinho/criar/(?P<estado>[A-Z]{2})/(?P<presidente>(\d{2}|XX))/(?P<governador>(\d{2}|XX))/(?P<senador>(\d{3}|XXX))/(?P<deputado_federal>(\d{4}|XXXX))/(?P<deputado_estadual>(\d{5}|XXXXX))$', 'santinho.views.criar', name='santinho_criar'),
